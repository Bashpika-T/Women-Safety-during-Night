...
def generate_data_description(save_dir):
    ...
    dataset['root'] = 'X:\\PETA\\out\\'
    dataset['image'] = []
    dataset['att'] = []
    dataset['att_name'] = []
    dataset['selected_attribute'] = range(35)
    # load PETA.MAT
    data = loadmat('X:\\PETA\\PETA.mat')
    ...
    with open(os.path.join(save_dir, 'peta_dataset.pkl'), 'wb') as f:  # Note the 'wb' mode
...
def create_trainvaltest_split(traintest_split_file):
    ...
    with open(traintest_split_file, 'wb') as f:  # Note the 'wb' mode
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='X:/PETA/peta_partition.pkl')
